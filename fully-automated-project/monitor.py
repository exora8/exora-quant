# ==============================================================================
# Exora Quant AI - v3.8 (MONITOR MODE)
# ==============================================================================
# This version is a MONITOR ONLY script. It does NOT execute any real trades.
# It simulates trade entries and exits based on the strategy, displays
# PnL in the terminal, and sends notifications via the Termux API.
# All other functions, including the web UI and backtester, remain the same.
# ==============================================================================
...
import hashlib
import os
import subprocess # MODIFIED: Added for Termux notifications
...
# --- HTML & JavaScript Template (No Changes) ---
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
...
# --- NEW: Termux Notification Helper ---
def send_termux_notification(title, content):
    """Sends a notification using the Termux API."""
    try:
        command = ['termux-notification', '--title', str(title), '--content', str(content)]
        subprocess.run(command, check=True, capture_output=True, text=True)
        app.logger.info(f"Sent notification: {title} - {content}")
    except FileNotFoundError:
        app.logger.warning("`termux-notification` command not found. Skipping notification.")
    except Exception as e:
        app.logger.error(f"Failed to send Termux notification: {e}")
        if hasattr(e, 'stderr'):
            app.logger.error(f"Termux API error: {e.stderr}")


# --- Data Fetching & Prediction (No Changes) ---
...
class BingXClient:
    ...
    def place_order(self, symbol, side, position_side, quantity, leverage):
        # ==============================================================================
        # --- MODIFIED FOR MONITOR MODE ---
        # This function no longer sends trades. It only logs the intent to trade.
        # ==============================================================================
        app.logger.info(f"[MONITOR MODE] Pretending to place {side} {position_side} order: {quantity:.5f} {symbol} @ {leverage}x")
        # Return a success-like dictionary so the rest of the bot logic continues as normal.
        return {"code": 0, "msg": "Monitor mode: Order not sent.", "data": {"orderId": int(time.time())}}
    
    def set_leverage(self, symbol, side, leverage):
        # In monitor mode, we can skip this network call entirely.
        app.logger.info(f"[MONITOR MODE] Pretending to set leverage for {symbol} {side} to {leverage}x")
        return {"code": 0}


# --- MODIFIED trade_bot_worker WITH NOTIFICATIONS ---
                ...
                if current_prices:
                    for item_id, position in active_positions_copy.items():
                        ...
                        if symbol in current_prices:
                            ...
                            if close_position:
                                app.logger.info(f"[SIMULATED CLOSE] {close_reason} hit for {symbol}. Closing {direction} position.")
                                # --- NOTIFICATION ---
                                send_termux_notification(f"{close_reason} Hit: {symbol}", f"Closing {direction} position. Price: {current_price}")
                                ...
                                if res and res.get('code') == 0:
                                    app.logger.info(f"Successfully simulated close for {symbol} due to {close_reason}.")
                                    ...
                                    app.logger.error(f"Failed to simulate close for {symbol} on {close_reason}: {res.get('msg') if res else 'Unknown error'}")
                                continue
            ...
            for item in trade_list_copy:
                try:
                    ...
                    if position_data: # Position Management (Reversal Signal)
                        ...
                        if signal_reversed:
                            app.logger.info(f"[SIMULATED CLOSE] Signal reversed for {symbol}. Closing {direction} position.")
                            # --- NOTIFICATION ---
                            send_termux_notification(f"Reversal Close: {symbol}", f"Signal reversed. Closing {direction} position.")
                            ...
                            if res and res.get('code') == 0:
                                app.logger.info(f"Successfully simulated reversal close for {symbol} position.")
                                with positions_lock, status_lock:
                                    if item_id in ACTIVE_POSITIONS: del ACTIVE_POSITIONS[item_id]
                                    BOT_STATUS[item_id] = {"message": "Waiting...", "color": "#fff", "last_close_time": time.time()}
                            else: app.logger.error(f"Failed to simulate reversal close for {symbol}: {res.get('msg') if res else 'Unknown error'}")
                        ...
                        if abs(price_change_pct) > trigger_percentage:
                            ...
                            if abs(current_price - sl_price) > 0:
                                ...
                                app.logger.info(f"[SIMULATED TRADE] Entry signal for {symbol}. Placing {direction} order.")
                                res = client.place_order(symbol, order_side, position_side, quantity, leverage)
                                
                                if res and res.get('code') == 0:
                                    app.logger.info(f"Successfully simulated opening of {direction} position for {symbol}.")
                                    # --- NOTIFICATION ---
                                    send_termux_notification(f"New Trade: {direction.upper()} {symbol}", f"Entry: {current_price:.4f}\nTP: {tp_price:.4f}, SL: {sl_price:.4f}")
                                    with positions_lock:
                                    ...
                                    app.logger.error(f"Failed to simulate opening position for {symbol}: {res.get('msg') if res else 'Unknown error'}")
                except Exception as e:
...
# --- NEW: Terminal Display Worker ---
def display_pnl_on_terminal():
    """Clears the terminal and displays a summary of active positions and PnL."""
    while True:
        time.sleep(2) # Refresh rate for the display
        os.system('cls' if os.name == 'nt' else 'clear')

        print("="*75)
        print(f" Exora Quant AI v3.8 - MONITOR MODE ".center(75, "="))
        print(f" Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ".center(75))
        print("="*75)
        print("\n-- Active Positions (Simulated) --\n")

        with positions_lock, status_lock:
            active_positions_copy = dict(ACTIVE_POSITIONS)
            bot_status_copy = dict(BOT_STATUS)

        if not active_positions_copy:
            print("No active positions.")
        else:
            # Manually formatted table header
            print(f"{'Symbol':<15} {'Direction':<10} {'Entry Price':<15} {'PnL (USDT)':<15} {'PnL (%)':<10}")
            print("-" * 75)
            for item_id, position in active_positions_copy.items():
                symbol = position.get('symbol', 'N/A')
                direction = position.get('direction', 'N/A').upper()
                entry_price = f"{position.get('entry_price', 0):.4f}"
                pnl = "-"
                pnl_pct = "-"

                status = bot_status_copy.get(item_id)
                if status and 'pnl' in status:
                    pnl_val = status['pnl']
                    pnl_pct_val = status['pnl_pct']
                    
                    # Add color using ANSI escape codes (optional, but nice for terminals)
                    color_start = "\033[92m" if pnl_val >= 0 else "\033[91m"
                    color_end = "\033[0m"
                    
                    pnl = f"{color_start}{pnl_val:.2f}{color_end}"
                    pnl_pct = f"{color_start}{pnl_pct_val:.2f}%{color_end}"


                print(f"{symbol:<15} {direction:<10} {entry_price:<15} {pnl:<22} {pnl_pct:<20}") # Adjusted padding for color codes

        print("\n" + "="*75)
        print("Web UI available at http://127.0.0.1:5000")
        print("NOTE: Trades are NOT being executed. This is a monitor only.")
        print("Detailed logs are being saved to bot_log.txt")


# --- Backtesting Engine (No Changes) ---
...
# --- Flask Routes (No Changes, but manual trades are also in monitor mode) ---
@app.route('/')
...
def manual_trade():
    try:
        ...
        app.logger.info(f"[MANUAL-SIM] Placing {side} order for {symbol}. Qty: {quantity:.4f}")
        res = client.place_order(symbol, order_side, position_side, quantity, lev) # Will be a simulated order
        if res and res.get('code') == 0:
            with positions_lock, status_lock:
            ...
            return jsonify({"message": f"MONITOR: Simulated manual {side} order for {symbol}."})
...
def manual_close():
    try:
        ...
        app.logger.info(f"[MANUAL-CLOSE-SIM] Closing {pos['direction']} for {symbol}. Qty: {pos['quantity']:.4f}")
        res = client.place_order(symbol, order_side, position_side, pos['quantity'], lev) # Will be a simulated order
        if res and res.get('code') == 0:
            with positions_lock, status_lock:
                if item_id in ACTIVE_POSITIONS: del ACTIVE_POSITIONS[item_id]
                BOT_STATUS[item_id] = {"message": "Waiting...", "color": "#fff", "last_close_time": time.time()}
            return jsonify({"message": f"MONITOR: Simulated close order for {symbol}."})
...
if __name__ == '__main__':
    # MODIFIED: Configure logging to save to a file to keep the terminal clean for the PnL display
    logging.basicConfig(level=logging.INFO,
                        format='%(asctime)s - %(levelname)s - %(message)s',
                        handlers=[
                            logging.FileHandler("bot_log.txt"),
                            # logging.StreamHandler() # Commented out to prevent logging to console
                        ])
    ...
    # --- NEW: Start the terminal PnL display thread ---
    threading.Thread(target=display_pnl_on_terminal, daemon=True).start()
