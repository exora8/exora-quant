...
<head>
    ...
    <title>Exora Quant AI v3.8 (High-Precision Backtest)</title>
    <style>
        ...
        .bt-control-group label { color: #bbb; min-width: 55px; }
...
# --- Backtesting Engine (RE-ENGINEERED for high-precision entries/exits) ---
...
def run_backtest_simulation(symbol, interval, start_ts, end_ts, leverage, starting_equity, risk_percentage, trigger_percentage):
    # 1. Fetch Data
    ...
    app.logger.info(f"Backtest: Fetching sub-interval {sub_interval} candles...")
    all_sub_candles_raw = []
    current_start_ts = start_ts
    while current_start_ts < end_ts:
    ...
    # 3. High-Precision Simulation Loop (iterates on sub-candles)
    for sub_candle in sub_candles:
        # --- A. CONTINUOUS POSITION MANAGEMENT ---
        if open_position:
            ...
            if exit_price:
                ...
                equity += pnl
                trades.append({'exit_time': sub_candle['t'], 'direction': open_position['direction'], 'pnl': pnl, 'return_pct': return_pct, 'exit_reason': exit_reason})
                open_position = None
        
        # --- B. SIGNAL GENERATION ON MAIN CANDLE CLOSE ---
        # Check if this sub-candle marks the start of a new main candle period
        if sub_candle['t'] in main_candle_open_timestamps and not open_position:
            current_main_idx = ts_to_main_candle_idx.get(sub_candle['t'])
            if current_main_idx and current_main_idx >= 50:
                # Use the history of main candles *before* this new one
                history_slice = all_candles_raw[current_main_idx - 50 : current_main_idx]
                predicted = predict_next_candles(history_slice, 20)
                
                if predicted:
                    signal_price = float(history_slice[-1][4]) # Close of the last main candle
                    change = ((predicted[-1]['c'] - signal_price) / signal_price) * 100
                    
                    if abs(change) > trigger_percentage:
                        entry_price = sub_candle['o'] # PRECISE ENTRY
