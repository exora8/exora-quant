# ==============================================================================
# Exora Quant AI - v3.7.2 with Price Precision Fix
# ==============================================================================
# This version fixes a persistent "Signature Mismatch" error on automated orders.
# The root cause was identified as a floating-point precision issue where TP/SL
# prices were sent with too many decimal places. The server would round these
# values before verifying the signature, causing a mismatch. The TP/SL prices
# are now explicitly rounded to a fixed precision before the request is sent.
# ==============================================================================
...
<head>
    ...
    <title>Exora Quant AI v3.7.2</title>
    <style>
...
class BingXClient:
    ...
    def _request(self, method, path, params=None):
        ...
        # NOTE: Sorting parameters is still correct and necessary.
    ...
    def place_order(self, symbol, side, position_side, quantity, leverage, tp_price=None, sl_price=None):
        ...
        # ============================ FIX STARTS HERE ============================
        # Round TP/SL prices to a reasonable precision (e.g., 5 decimal places) to avoid
        # signature mismatch errors caused by the server truncating long float values.
        if tp_price:
            params["takeProfit"] = f'{{"type":"TAKE_PROFIT_MARKET","stopPrice":{tp_price:.5f}}}'
        if sl_price:
            params["stopLoss"] = f'{{"type":"STOP_LOSS_MARKET","stopPrice":{sl_price:.5f}}}'
        # ============================= FIX ENDS HERE =============================
                ...
                # --- Position Entry Logic ---
                    ...
                    if abs(price_change_pct) > 1.5:
                        ...
                        if abs(current_price - sl_price) > 0:
                            ...
                            res = client.place_order(symbol, order_side, position_side, quantity, leverage, tp_price, sl_price)
                            
                            if res and res.get('code') == 0:
                                app.logger.info(f"Successfully opened {direction} position for {symbol}.")
                                with positions_lock:
                                    ACTIVE_POSITIONS[item_id] = {'symbol': symbol, 'quantity': quantity, 'direction': direction, 'entry_price': current_price}
                            else:
