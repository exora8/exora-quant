...
# THIS VERSION FORCES the backtest sub-interval to 1-minute for maximum resolution and accuracy.
# ==============================================================================
...
<head>
    ...
    <title>Exora Quant AI v3.8 (Max-Resolution Backtest)</title>
    <style>
        ...
        #backtest-panel { width: 500px; display: flex; flex-direction: column; }
...
def trade_bot_worker():
    ...
    while True:
        ...
        try:
                ...
                if current_prices:
                    for item_id, position in active_positions_copy.items():
                        ...
                        if symbol in current_prices:
                            current_price, tp, sl = current_prices[symbol], position.get('tp_price'), position.get('sl_price')
                            ...
                            if close_position:
                                app.logger.info(f"Closing {symbol} {direction} position due to {close_reason}.")
                                res = client.place_order(symbol, "SELL" if direction == 'long' else "BUY", direction.upper(), position['quantity'], leverage)
            ...
            if time.time() - last_analysis_time < analysis_interval: continue
            last_analysis_time = time.time()
            
            balance_response = client.get_balance()
            if not (balance_response and balance_response.get('code') == 0):
            ...
            for item in trade_list_copy:
                try:
                    ...
                    if item_id in ACTIVE_POSITIONS: continue # Already in a position for this item
                    
                    with status_lock:
                        if time.time() - BOT_STATUS.get(item_id, {}).get('last_close_time', 0) < TRADE_COOLDOWN_SECONDS:
                            continue
                    ...
                    if abs(price_change_pct) > trigger_percentage:
                        ...
                        if abs(current_price - sl_price) > 0:
                            ...
                            res = client.place_order(symbol, "BUY" if direction == 'long' else "SELL", direction.upper(), quantity, leverage)
                            if res and res.get('code') == 0:
                                with positions_lock:
                    ...
                    app.logger.error(f"Error in analysis for {item.get('symbol', 'N/A')}: {e}")
        except Exception as e:
...
def pnl_updater_worker():
    while True:
        ...
        try:
            ...
            app.logger.error(f"Error in PnL updater worker: {e}")

# --- Backtesting Engine (Forced 1-min sub-interval for max accuracy) ---
def get_sub_interval(interval):
    """MODIFIED: Always returns 1-minute interval for maximum backtest resolution."""
    return "1"

def run_backtest_simulation(symbol, interval, start_ts, end_ts, leverage, starting_equity, risk_percentage, trigger_percentage, dca_frequency, dca_amount):
    app.logger.info(f"Backtest: Fetching primary {interval} candles...")
    all_candles_raw = []
    ...
    app.logger.info(f"Backtest: Fetching sub-interval {sub_interval} candles for max resolution...")
    all_sub_candles_raw = []
    current_start_ts = start_ts
    while current_start_ts < end_ts:
    ...
    equity_curve, trades, open_position = [{'time': start_ts, 'equity': starting_equity}], [], None
    ...
    for sub_candle in sub_candles:
        ...
        if dca_frequency != 'none' and dca_amount > 0:
            perform_dca = False
            if dca_frequency == 'weekly' and current_ts - last_dca_time >= WEEK_MS:
                perform_dca = True
            elif dca_frequency == 'monthly':
                current_date = datetime.fromtimestamp(current_ts / 1000)
                last_dca_date = datetime.fromtimestamp(last_dca_time / 1000)
                if current_date.month != last_dca_date.month or current_date.year != last_dca_date.year:
                    perform_dca = True
            if perform_dca:
                equity += dca_amount
        ...
        if current_ts in main_candle_open_timestamps and not open_position:
    ...
    net_profit = equity - starting_equity - total_dca_added
    total_trades = len(trades); win_rate = (len([t for t in trades if t['pnl'] > 0]) / total_trades * 100) if total_trades > 0 else 0; total_profit = sum(t['pnl'] for t in trades if t['pnl']>0); total_loss = abs(sum(t['pnl'] for t in trades if t['pnl']<=0)); profit_factor = total_profit / total_loss if total_loss > 0 else float('inf'); max_dd, peak = 0, starting_equity
...
def get_balance():
    try:
        ...
        if balance_data and balance_data.get('code') == 0:
            return jsonify({"total_balance": float(balance_data['data']['balance']['balance'])})
        else:
...
def manual_trade():
    try:
        ...
        if price_diff_per_unit == 0: return jsonify({"error": "Price difference is zero"}), 400
        quantity = risk_usdt / price_diff_per_unit
        res = client.place_order(symbol, "BUY" if side == 'long' else "SELL", "LONG" if side == 'long' else "SHORT", quantity, lev)
        if res and res.get('code') == 0:
            with positions_lock:
                ACTIVE_POSITIONS[item_id] = {'symbol': symbol, 'quantity': quantity, 'direction': side, 'entry_price': current_price}
            return jsonify({"message": f"Manual {side} order placed."})
        return jsonify({"error": f"Failed: {res.get('msg') if res else 'Unknown'}"}), 400
    except Exception as e: return jsonify({"error": str(e)}), 500

@app.route('/api/manual_close', methods=['POST'])
def manual_close():
    try:
        data = request.json; item_id = data['id']
        with positions_lock:
            if item_id not in ACTIVE_POSITIONS: return jsonify({"message": "No active position."}), 404
            pos = ACTIVE_POSITIONS[item_id]
        with settings_lock: 
            client = BingXClient(SETTINGS['bingx_api_key'], SETTINGS['secret_key'], SETTINGS['mode'] == 'demo')
            lev = SETTINGS['leverage']
        res = client.place_order(pos['symbol'], "SELL" if pos['direction'] == 'long' else "BUY", pos['direction'].upper(), pos['quantity'], lev)
        if res and res.get('code') == 0:
            with positions_lock, status_lock:
                if item_id in ACTIVE_POSITIONS: del ACTIVE_POSITIONS[item_id]
                BOT_STATUS[item_id] = {"message": "Waiting...", "color": "#fff", "last_close_time": time.time()}
            return jsonify({"message": f"Close order for {pos['symbol']} placed."})
        return jsonify({"error": f"Failed: {res.get('msg') if res else 'Unknown'}"}), 400
    except Exception as e: return jsonify({"error": str(e)}), 500
...
def handle_backtest():
    ...
    try:
        ...
        results = run_backtest_simulation(
            ...
            data.get('leverage', 10), data.get('equity', 10000.0),
            data.get('risk_percentage', 1.0), data.get('trigger_percentage', 4.0),
            data.get('dca_frequency', 'none'), data.get('dca_amount', 0.0)
        )
